...
async function ensurePostgres(): Promise<boolean> {
  try {
    ...
    if (!isPostgresQuotaOrUnavailableError(error)) {
      console.error("[Sooqna Notify] notification table ensure failed", error);
    }
    return false;
  }
}
...
async function readJson(): Promise<AppNotification[]> {
  ...
  jsonCache = await loadCollection<AppNotification>(FILE);
  return jsonCache;
}
...
export async function createNotification(
  ...
  if (await ensurePostgres()) {
    try {
      ...
      // Neon quota / connectivity — fall through to JSON so events are not dropped.
    }
  }
